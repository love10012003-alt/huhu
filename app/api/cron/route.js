...
export async function GET(){
  ...
  try{
    ...
    if(j.error) return NextResponse.json({ok:false,domain:'f.lal.vn',error:j.error.code+' - '+j.error.message},{status:500})
    ...
    if(flights.length===0) return NextResponse.json({ok:false,domain:'f.lal.vn',error:'0 chuyen',raw:j})
    ...
    return NextResponse.json({ok:true,domain:'f.lal.vn',count:flights.length})
